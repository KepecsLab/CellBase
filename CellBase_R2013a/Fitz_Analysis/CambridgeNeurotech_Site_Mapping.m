%% Neuralynx, cambridge neurotech setup...
...
% trodes
table_sorted_P2 = sortrows(probe_mapping_P2, 'cambridge'); % order table by cambridge indices
...
% make 8 x 2 x 2 matrix (8 sites by 2 columns trodes by x,y) to hold x,y coordinates of each site on a shank
P2_geo = zeros(8,2,2);
% x coordinates
...
% example- access neuralynx channel numbers corresponding to E2 probe
% trodes
table_sorted_E2 = sortrows(probe_mapping, 'cambridge'); % order table by cambridge indices
table_sorted_E2(E2_trodes, :)  % display rows comprising (in order) TT1 - TT8

%% make E2 geom.csv file and define trode ncs file number to ms4 trode site number mapping

% put cambridge neurotech site IDs into 4 columns where columns 1:2
% correspond to shank A, and coumns 3:4 correspond to shank B
E2_trodes_ms4 = [16 12 8 6 2 3 5 9 15 10 14 13 4 1 7 11 ...
    17 21 22 25 27 29 28 30 18 23 19 20 24 26 31 32];
E2_trodes_ms4 = E2_trodes_ms4(:);
E2_trodes_ms4 = reshape(E2_trodes_ms4, 8, 4);

% x, y coordinates for each shank (complicated geometry, computed by hand)
E2_geo = [...
    0 0;
    3 40;
    6 80;
...
trodeIx = [ones(16,1); (ones(16,1) + 1)];
trodeSiteIx = (1:32)'; 
x = E2_geo(:,1); x = repmat(x, 2, 1);
y = E2_geo(:,2); y = repmat(y, 2, 1);

E2_ms4_geo = table(trodeIx, trodeSiteIx, table_sorted_E2.neuralynx(E2_trodes_ms4(:)),...
    x, y, 'VariableNames', {'trode', 'site', 'ncs', 'x', 'y'});

% make geom.csv file (nsites x (x,y))
E2_ms4_geo = sortrows(E2_ms4_geo, 'site');

% write the csv file
toCsv = zeros(16,2);
toCsv(:,1) = E2_ms4_geo.x(1:16);
toCsv(:,2) = E2_ms4_geo.y(1:16);

[fname, pname] = uiputfile('path', 'Choose geometry file base directory');
pname = [pname 'E2' filesep];
ensureDirectory(pname);
csvwrite(fullfile(pname, 'geo.csv'), toCsv);
disp(['*** saving geo.csv in ' pname ' ***']);

% make trode creation file mapping .mat file (nSites x nTrodes)
E2_ms4_geo = sortrows(E2_ms4_geo, {'trode', 'site'});
toMat = zeros(16,2);
toMat(:) = E2_ms4_geo.ncs;
save(fullfile(pname, 'ncs2ms4_map.mat'), 'toMat');