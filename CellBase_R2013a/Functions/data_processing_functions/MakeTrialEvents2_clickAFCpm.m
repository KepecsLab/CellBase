function MakeTrialEvents2_clickAFCpm(sessionpath)
...
try
    load([sessionpath 'EventTTL_task.mat']);   % load Neuralynx events
catch    
...
SE_filename = [sessionpath filesep 'SessionData.mat'];
TE = load(SE_filename);

% Create cell aray to group TTLs adn their timestamps by trial. 
[NlxEvents] = GetTrialStartTimeStamp(EventTTL_task, EventTimestamps);

% Set TTl alignement state at 2 (Trial start)
idx=2; 
...
TE2 = TE.TE;
son2 = EventTimestamps(son)/(10^6);   % stimulus onset time recorded by the recording system (Neuralynx)
ts = TE2.TrialStartTimeStamp;   % stimulus onset in absolut time recorded by the behavior control system
